...
//Definition done by Ben + ChatGPT
  .clk(sys_clk),
  .reset(((sys_rst | mgmtsoc_reset) | mgmtsoc_vexriscv_debug_reset)),
  
  // Data Bus Interface (dBus)
  .dBus_cmd_valid(mgmtsoc_dbus_dbus_stb), // Command valid
  .dBus_cmd_ready(mgmtsoc_dbus_dbus_ack), // Command ready
  .dBus_cmd_payload_wr(mgmtsoc_dbus_dbus_we), // Write enable
  .dBus_cmd_payload_uncached(1'b0), // Uncached option (if needed)
  .dBus_cmd_payload_address(mgmtsoc_dbus_dbus_adr), // Address
  .dBus_cmd_payload_data(mgmtsoc_dbus_dbus_dat_w), // Data
  .dBus_cmd_payload_mask(mgmtsoc_dbus_dbus_sel), // Mask
  .dBus_cmd_payload_size(3'b0), // Size (if needed, adjust accordingly)
  .dBus_cmd_payload_last(1'b0), // Last (adjust accordingly)
  
  .dBus_rsp_valid(mgmtsoc_dbus_dbus_ack), // Response valid
  .dBus_rsp_payload_last(1'b0), // Last (adjust accordingly)
  .dBus_rsp_payload_data(mgmtsoc_dbus_dbus_dat_r), // Response data
  .dBus_rsp_payload_error(mgmtsoc_dbus_dbus_err), // Error
  
  // Interrupts
  .timerInterrupt(1'b0), // Timer interrupt (set to 0 as per your original config)
  .externalInterrupt(mgmtsoc_interrupt), // External interrupt
  .softwareInterrupt(1'b0), // Software interrupt (set to 0 as per your original config)
  
  // Debug Bus Interface
  .debug_bus_cmd_valid(mgmtsoc_vexriscv_i_cmd_valid), // Command valid
  .debug_bus_cmd_ready(mgmtsoc_vexriscv_o_cmd_ready), // Command ready
  .debug_bus_cmd_payload_wr(mgmtsoc_vexriscv_i_cmd_payload_wr), // Write enable
  .debug_bus_cmd_payload_address(mgmtsoc_vexriscv_i_cmd_payload_address), // Address
  .debug_bus_cmd_payload_data(mgmtsoc_vexriscv_i_cmd_payload_data), // Data
  
  .debug_resetOut(mgmtsoc_vexriscv_o_resetOut), // Reset output
  .debug_bus_rsp_data(mgmtsoc_vexriscv_o_rsp_data), // Response data
  
  // Instruction Bus Interface (iBus)
  .iBus_cmd_valid(mgmtsoc_ibus_ibus_stb), // Command valid
  .iBus_cmd_ready(mgmtsoc_ibus_ibus_ack), // Command ready
  .iBus_cmd_payload_address(mgmtsoc_ibus_ibus_adr), // Address
  .iBus_cmd_payload_size(3'b0), // Size (adjust accordingly)
  
  .iBus_rsp_valid(mgmtsoc_ibus_ibus_ack), // Response valid
  .iBus_rsp_payload_data(mgmtsoc_ibus_ibus_dat_r), // Response data
  .iBus_rsp_payload_error(mgmtsoc_ibus_ibus_err) // Error
);
